...
import UseInsertNilaiKdsKeterampilan from "../lib/mutation/UseInsertNilaiKdsKeterampilan";

export default function AddGradKeterampilan() {
  const { id, mapel } = useParams();

  const { data: data1 } = UseGetNameUserById(id);
  const { data: mapelData } = UseGetMapelDetail(mapel, id);
  const { InsertData, data: result, error } = UseInsertNilaiKdsKeterampilan();
      ...
      const nilai1 = mapelData.e_raport_mapel_by_pk.temas.map((item) => {
        const nilai = item.kds.map((item) => {
          data.push({
            ...
            keterampilan:
              item.nilai_kds.length === 0 ? 0 : item.nilai_kds[0].keterampilan,
          });
          console.log(data);
          return 0;
        });
  ...
  const handleChange = (e, id) => {
    ...
    const newArr = data.map((item, key) => {
      if (item.kd_id === id) {
        return { ...item, keterampilan: value };
      }
      return item;
    });
    setData(newArr);
      ...
      >
        <Box style={{ padding: "32px" }} overflow="auto">
          <Typography variant="h5" display={"flex"} flexWrap="wrap">
            ...
            <Box color="#757575"> &nbsp; {` > keterampilan`}</Box>
          </Typography>
